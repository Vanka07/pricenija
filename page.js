...
import { 
  ...
  Plus, Save, X, Check, TrendingUp, TrendingDown, Calendar, 
  Download, RefreshCw, Bell, Menu, LogOut
} from 'lucide-react';
    ...
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-stone-900 border-r border-stone-800 z-50 transform transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          {menuItems.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                activeSection === item.id ? 'bg-emerald-500/20 text-emerald-400' : 'text-stone-400 hover:text-white hover:bg-stone-800/50'
              }`}
            >
              <item.icon size={20} />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-stone-800">
          <a href="/" className="flex items-center gap-2 p-3 rounded-xl bg-stone-800/50 text-stone-400 hover:text-white transition-all">
            <Home size={18} />
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-white">
          ...
          <div className="flex items-center justify-between px-4 sm:px-6 h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">{new Date().toLocaleDateString('en-NG', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-stone-900/50 rounded-2xl border border-stone-800 p-6">
                <div className="flex flex-col sm:flex-row gap-4">
                  <div className="flex-1">
                    ...
                    <select value={selectedMarket} onChange={(e) => setSelectedMarket(parseInt(e.target.value))} className="w-full px-4 py-3 bg-stone-800 border border-stone-700 rounded-xl text-white focus:ring-2 focus:ring-emerald-500 outline-none">
                  ...
                  <div className="flex-1">
                    ...
                    <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="w-full px-4 py-3 bg-stone-800 border border-stone-700 rounded-xl text-white focus:ring-2 focus:ring-emerald-500 outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-stone-900/50 rounded-2xl border border-stone-800 overflow-hidden">
                <div className="p-4 border-b border-stone-800 flex items-center justify-between">
                  <h3 className="font-semibold text-white">Enter Prices - {markets.find(m => m.id === selectedMarket)?.name}</h3>
                  <button onClick={savePrices} className="flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600">
                            ...
                            <div key={commodity.id} className="flex items-center gap-4 p-3 rounded-xl bg-stone-800/20">
                              ...
                              <div className="w-40">
                                <div className="relative">
                                  ...
                                  <input type="number" value={priceEntries[commodity.id] || ''} onChange={(e) => handlePriceChange(commodity.id, e.target.value)} placeholder="0" className="w-full pl-8 pr-4 py-2 bg-stone-800 border border-stone-700 rounded-lg text-white text-right focus:ring-2 focus:ring-emerald-500 outline-none" />
                                </div>
                              </div>
                            </div>
                          ))}
                  ...
                  <tbody className="divide-y divide-stone-800/50">
                    {commodities.map(item => (
                      <tr key={item.id} className="hover:bg-stone-800/30">
                        ...
                        <td className="p-4"><span className="px-2 py-1 rounded-full text-xs bg-emerald-500/20 text-emerald-400">Active</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-stone-900/50 rounded-2xl border border-stone-800 p-6">
                  ...
                  <button className="w-full py-2 bg-emerald-500/20 text-emerald-400 rounded-lg hover:bg-emerald-500/30">Download CSV</button>
                </div>
                <div className="bg-stone-900/50 rounded-2xl border border-stone-800 p-6">
                  ...
                  <button className="w-full py-2 bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500/30">Select Dates</button>
                </div>
              </div>
            </div>
          )}
      ...
      {toast && (
        <div className="fixed bottom-4 right-4 z-50 flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-500 text-white shadow-lg">
          <Check size={20} />
          <span>{toast}</span>
          <button onClick={() => setToast(null)} className="p-1 hover:bg-white/20 rounded"><X size={16} /></button>
        </div>
      )}
    </div>
  );
