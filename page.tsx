...
const generateHistoricalData = (commodityId: number, days = 30) => {
...
const PriceChange = ({ change, size = 'default' }: { change: number; size?: string }) => {
...
export default function HomePage() {
  ...
  const avgPrice = Math.round(
    commodityPrices.reduce((sum, p) => sum + p.price, 0) / commodityPrices.length
  );
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                ...
                <p className="text-stone-400 mt-1">
                  Real-time prices across Nigeria&apos;s top markets
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-5 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-emerald-600/5 border border-emerald-500/30">
                <p className="text-stone-400 text-sm">Avg. Grain Price</p>
                <p className="text-2xl font-bold text-white mt-1">{formatCompactPrice(72000)}</p>
                <p className="text-xs text-stone-500 mt-1">per 100kg bag</p>
              </div>
              <div className="p-5 rounded-2xl bg-gradient-to-br from-blue-500/20 to-blue-600/5 border border-blue-500/30">
                <p className="text-stone-400 text-sm">Best Market</p>
                <p className="text-2xl font-bold text-white mt-1">Dawanau</p>
                <p className="text-xs text-stone-500 mt-1">Kano, North-West</p>
              </div>
              <div className="p-5 rounded-2xl bg-gradient-to-br from-rose-500/20 to-rose-600/5 border border-rose-500/30">
                <p className="text-stone-400 text-sm">Most Volatile</p>
                <p className="text-2xl font-bold text-white mt-1">Tomatoes</p>
                <p className="text-xs text-rose-400 mt-1">+12.5% this week</p>
              </div>
              <div className="p-5 rounded-2xl bg-gradient-to-br from-amber-500/20 to-amber-600/5 border border-amber-500/30">
                <p className="text-stone-400 text-sm">Watchlist Items</p>
                <p className="text-2xl font-bold text-white mt-1">{watchlist.length}</p>
                <p className="text-xs text-stone-500 mt-1">commodities tracked</p>
              </div>
            </div>
                ...
                <div className="space-y-3">
                  {watchlist.slice(0, 5).map((id) => {
                    ...
                    return (
                      <div
                        key={id}
                        className="flex items-center gap-3 p-3 rounded-xl bg-stone-800/30 hover:bg-stone-800/50 transition-all cursor-pointer"
                        onClick={() => {
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-stone-500">
                            Avg: {formatCompactPrice(avgPriceVal)}
                          </p>
                        </div>
            ...
            <div className="bg-stone-900/50 rounded-2xl border border-stone-800/50 p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {markets.map((market) => {
                  const index =
                    market.region === 'North-West'
                      ? 85
                      : market.region === 'North-Central'
                      ? 95
                      : market.region === 'South-West'
                      ? 112
                      : market.region === 'South-East'
                      ? 108
                      : 100;
                  ...
                  return (
                    <div
                      key={market.id}
                      className="p-4 rounded-xl bg-stone-800/30 text-center"
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-right">
                            <p className="font-bold text-white">
                              {formatCompactPrice(avgPriceVal)}
                            </p>
                            <PriceChange change={avgChange} />
                          </div>
                        </div>
              ...
              <div className="bg-stone-900/50 rounded-2xl border border-stone-800/50 p-6">
                ...
                <div className="space-y-3">
                  {commodityPrices.map((item, index) => {
                    const vsAvg = ((item.price - avgPrice) / avgPrice) * 100;
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-stone-500 flex items-center gap-1">
                            <MapPin size={12} />
                            {item.market?.city}, {item.market?.state}
                          </p>
                        </div>
                        <div className="text-right">
                          <p
        ...
        {activeTab === 'markets' && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-stone-400 mt-1">
                Major commodity markets across Nigeria
              </p>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {markets.map((market) => {
                ...
                const avgChange =
                  marketPrices.reduce((sum, p) => sum + p.change, 0) /
                  marketPrices.length;

                return (
                  <div
                    key={market.id}
                    className="bg-stone-900/50 rounded-2xl border border-stone-800/50 p-5 hover:border-stone-700/50 transition-all"
                  >
            ...
            {watchlist.length === 0 ? (
              <div className="bg-stone-900/50 rounded-2xl border border-stone-800/50 p-12 text-center">
                ...
                <p className="text-stone-500 mb-4">
                  Star commodities to add them to your watchlist
                </p>
                <button
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {watchlist.map((id) => {
                  ...
                  const avgChange =
                    prices.reduce((sum, p) => sum + p.change, 0) / prices.length;
                  const lowestPriceItem = prices.reduce(
                    (min, p) => (p.price < min.price ? p : min),
                    prices[0]
                  );
                  const lowestMarket = markets.find(
                    (m) => m.id === lowestPriceItem?.marketId
                  );

                  return (
                    <div
                      key={id}
                      className="bg-stone-900/50 rounded-2xl border border-stone-800/50 p-5"
                    >
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <span className="text-3xl">{commodity?.icon}</span>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                          ...
                          <p className="text-lg font-bold text-emerald-400">
                            {formatPrice(lowestPriceItem?.price)}
                          </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <p className="text-sm text-stone-500 text-center">
              Empowering farmers, traders, and consumers with real-time market
              intelligence
            </p>
